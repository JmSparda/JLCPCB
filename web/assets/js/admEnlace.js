...
$(function(){
   $('tr #btneliminar').click(function(e){
      e.preventDefault();
      var opcion = confirm("Desea Eliminar al Enlace");
      
      if(opcion){
        var fila = $(this).parent().parent();
        var idenlace = fila.find('#id_enlace').text();
        var data = {idEnlace : idenlace};
        $.post("deleteEnlace", data, function(res,est,jqXHR){
            //alert(res);
            fila.remove();
        }); 
      }
   ...
   $("#frmmodificar").validate({
       ...
       messages : { //CREAMOS LOS MENSAJES PARA CADA REGLA
          ...
          /* usu_usuario : {
           ...
           $('#btnmodificarenlace').click(function(e){
                e.preventDefault();
